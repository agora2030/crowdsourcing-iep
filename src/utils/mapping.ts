export const entityMap = new Map();
entityMap.set(1, {
  name: "una Empresa financiera",
  description:
    "como los bancos, utilizan datos para tomar decisiones de inversión y crédito más informadas y precisas. Pueden evaluar los riesgos y oportunidades, y ofrecer soluciones financieras.",
});
entityMap.set(2, {
  name: "una Empresa de mercado",
  description:
    "Pueden ser proveedores y/o dedicarse a procesos logísticos. Optimizan sus operaciones y mejorar la eficiencia en la cadena de suministro. Con datos sobre inventario, producción, envíos y entregas, estas empresas identifican patrones para ofrecer soluciones más efectivas y aumentar su competitividad en el mercado.",
});
// entityMap.set(3, {
//   name: "una Entidad pública",
//   description:
//     "Usan datos para tomar decisiones informadas sobre políticas públicas y servicios, lo que les permite mejorar la calidad de vida de la población. Las entidades públicas pueden identificar tendencias, patrones y áreas problemáticas, y crear soluciones efectivas que satisfagan las necesidades de la sociedad.",
// });

export const requestMap = new Map();
requestMap.set(1, {
  name: "Datos de registro de tu empresa.",
  description: "Como matricula, certificado de registro, licencias y permisos",
});
requestMap.set(2, {
  name: "Información transaccional.",
  description:
    "Aquella que registra las transacciones financieras y comerciales que realiza. Por ejemplo, compras, ventas, pagos y cobros",
});
requestMap.set(3, {
  name: "Datos de tus clientes.",
  description:
    "Información que recopila y almacena sobre clientes, como nombres, direcciones, números de teléfono, correos electrónicos, preferencias de compra e historial de compras",
});
requestMap.set(4, {
  name: "Datos sociodemográficos.",
  description:
    "Información estadística sobre las características de la población, como edad, género, ingresos, educación y ubicación geográfica",
});
requestMap.set(5, {
  name: "Información de tus productos/servicios.",
  description:
    "Descripción detallada de productos o servicios que ofreces, incluyendo características, precios y beneficios",
});

export const resultMap = new Map();
resultMap.set(1, {
  name: "una Alianza.",
  description: "",
});
resultMap.set(2, {
  name: "Estudios e investigación.",
  description: "",
});
resultMap.set(3, {
  name: "un Perfilamiento o historiales.",
  description: "",
});
